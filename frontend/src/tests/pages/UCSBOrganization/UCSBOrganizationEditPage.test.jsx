  ...
  describe("tests where backend is working normally", () => {
    beforeEach(() => {
      ...
      axiosMock.onPut("/api/ucsborganizations").reply(200, {
        ...
        orgTranslation: "Gaucho Sports Analytics Club at UCSB",
        inactive: false,
      });
    });
    ...
    test("Is populated with the data provided", async () => {
      ...
      expect(orgTranslationShortField).toHaveValue("Gaucho Sports Analytics");
      expect(orgTranslationField).toBeInTheDocument();
      expect(orgTranslationField).toHaveValue("Gaucho Sports Analytics Club");
      ...
      fireEvent.change(orgTranslationField, {
        target: { value: "Gaucho Sports Analytics Club at UCSB" },
      });
      fireEvent.click(submitButton);

      await waitFor(() => expect(mockToast).toBeCalled());
      expect(mockToast).toBeCalledWith(
        "Organization Updated - orgCode: GSAC name: Gaucho Sports Analytics Club",
      );

      expect(mockNavigate).toBeCalledWith({ to: "/ucsborganization" });

      expect(axiosMock.history.put.length).toBe(1); // times called
      expect(axiosMock.history.put[0].params).toEqual({ orgCode: "GSAC" });
