...
public class RecommendationRequestsIT {
  ...
  @Autowired RecommendationRequestsRepository recommendationRequestsRepository;
  ...
  @Test
  public void test_logged_in_user_can_get_recommendation_request_by_id() throws Exception {
    // arrange
    ...
    RecommendationRequests rr =
        RecommendationRequests.builder()
            ...
            .dateRequested(LocalDateTime.parse("2025-01-01T09:00"))
            .dateNeeded(LocalDateTime.parse("2025-02-01T17:00"))
            .done(false)
            .build();

    recommendationRequestsRepository.save(rr);
  ...
  public void admin_user_can_create_recommendation_request() throws Exception {
    ...
    RecommendationRequests expected =
        RecommendationRequests.builder()
            .id(1L)
            .requesterEmail("bob@ucsb.edu")
            .professorEmail("advisor@ucsb.edu")
            .explanation("Internship")
            .dateRequested(LocalDateTime.parse("2025-03-10T10:30"))
            .dateNeeded(LocalDateTime.parse("2025-03-25T23:59"))
            .done(true)
            .build();
    ...
    MvcResult response =
        mockMvc
            .perform(
                post("/api/recommendationrequests/post?requesterEmail=bob@ucsb.edu"
                        + "&professorEmail=advisor@ucsb.edu"
                        + "&explanation=Internship"
                        + "&dateRequested=2025-03-10T10:30:00"
                        + "&dateNeeded=2025-03-25T23:59:00"
                        + "&done=true")
                    .with(csrf()))
            .andExpect(status().isOk())
            .andReturn();
